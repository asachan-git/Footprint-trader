"""Parse Bybit footprint payload (format=bybit_v1) → canonical Bar."""
...
def parse(payload: dict) -> Bar:
    ...
    return Bar(
        ...
        bid_ladder=tuple(Level(price=float(l["price"]), vol=float(l["vol"]), cnt=float(l.get("cnt", 0.0))) for l in payload.get("bid_ladder", [])),
        ask_ladder=tuple(Level(price=float(l["price"]), vol=float(l["vol"]), cnt=float(l.get("cnt", 0.0))) for l in payload.get("ask_ladder", [])),
        poc=payload.get("poc"),
