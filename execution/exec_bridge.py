        ...
        if (not cv_active and reason is None
            ...
            if (activate > 0 and peak >= activate and net_pnl > 0
                ...
                _emit_exit_audit({"account": str(account), "broker_symbol": symbol,
                                  ...
                                  "squeeze_ok": cyc.get("squeeze_ok"),
                                  "squeeze_rank": cyc.get("squeeze_rank")})
                # partial exit — the cycle lives on, so this is recorded as its own
                # row (partial=True) and the eventual flatten writes a second row.
                _emit_cycle_outcome(cyc, account=str(account), symbol=symbol, magic=magic,
                                    tf=tf, exit_reason="bias_book_trail", partial=True,
                                    bias=bias, peak=round(peak, 2),
